...
Statistics* computeStatistics(float dataset[], int size) {
	...
	if (size > 1) {	
		...
		fprintf(output,"WARNING: Not enough data to compute Standard Deviation\n");
		s->popStdDev = 0;
		s->smplStdDev = 0;	
	}
...
void writeStatistics(Interval interval, Statistics* overall) {
	...
	fprintf(output, "Cars through intersection: %d\n", interval.numberOfCars);
	fprintf(output, "Minimum Detected Speed: %f\n", interval.stats->min);
	fprintf(output, "Maximum Detected Speed: %f\n", interval.stats->max);
	fprintf(output, "Average Detected Speed: %f\n", interval.stats->avg);
	fprintf(output, "Sample Standard Deviation: %f\n", interval.stats->smplStdDev);
	fprintf(output, "Population Standard Deviation: %f\n", interval.stats->popStdDev);
	fprintf(output, "Mode(s): ");

	for (int i = 0; i < interval.stats->mode->numModes; i++) {
		if(i != interval.stats->mode->numModes) {
			fprintf(output, "%f, ", interval.stats->mode->modes[i]);
		} else {
			fprintf(output, "%f ", interval.stats->mode->modes[i]);
		}
	}

	fprintf(output, "(Appears %d times)\n", interval.stats->mode->numModes);

	fprintf(output, "------------------------------------------------------\n");
	fprintf(output, "Overall Average Speed: %f\n", overall->avg);
	fprintf(output, "Average Speed is SLOWEST during Interval %d (%f)\n", overall->minIndex, overall->min);
	fprintf(output, "Average Speed is FASTEST during Interval %d (%f)\n", overall->maxIndex, overall->max);
	if (overall->popStdDev != 0) {
		fprintf(output, "Overall Sample Standard Deviation: %f\n", overall->smplStdDev);
		fprintf(output, "Overall Population Standard Deviation: %f\n\n\n", overall->popStdDev);
	}
...
int main() {
	...
	fprintf(output,"SMART TRAFFIC BEGINNING\n");
  
  	for (int currentInterval = 0; currentInterval < maxIntervals; currentInterval++) {
		trigger = 15000;
      		if (road1.active) {
			fprintf(output, "------- ROAD 1: Interval %d -------\n", (currentInterval/2)+1);
			fprintf(output,"Switching Red Lights\n");
        		gpio_request(red2, NULL);
			...
			fprintf(output, "------- ROAD 2: Interval %d -------\n", (currentInterval/2)+1);
			fprintf(output,"Switching Red Lights\n");
        		gpio_request(red1, NULL);
			...
			delay(3000);
      		...
      		do {
				...
				if (value3 == 1 && (switching == false)) {
					switching = true;
					fprintf(output, "Car approached from other intersection, Changing lights shortly\n");
					begin = clock();
					elapsed = 0;
					trigger = 1500;				
				}
          			...
          			if (value1 == 1 && (pressed == false)) {
              				tStarts = clock();
					fprintf(output,"Car entered\n");
              				pressed = true;
            			} else if (value2 == 1 && (pressed == true)) {
            				tEnds = clock();
					fprintf(output,"Car left\n");
		...
		//fclose(output);
			...
			if (road1.intervals[currentInterval/2].numberOfCars > 0) {
				...
				fprintf(output,"WARNING: No cars passed during Interval %d and no stats were recorded.\n", (currentInterval/2) + 1);			
			}
      			road1.active = false;
        		road2.active = true;
      		} else {
